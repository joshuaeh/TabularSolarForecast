...
class TabularTest:
    ...
    def import_preprocess_cached_windows(self, train_validate_date, end_date, verbose):
        ...
        self.selected_past_features = [
            feature
            for feature in self.selected_features
            if feature in self.all_past_features
        ]
        self.selected_future_features = [
            feature
            for feature in self.selected_features
            if feature in self.all_future_features
        ]
        self.selected_scalar_responses = [
            response
            for response in self.selected_responses
            if response in self.all_scalar_responses
        ]
        self.selected_relative_responses = [
            response
            for response in self.selected_responses
            if response in self.all_relative_responses
        ]
        ...
        if verbose >= 2:
            ...
            print(
                f"# selected relative responses: {len(self.selected_relative_responses)}"
            )

        # get indices of selected quantities
        selected_past_features_indices = [
            self.all_past_features.index(feature)
            for feature in self.selected_past_features
        ]
        selected_future_features_indices = [
            self.all_future_features.index(feature)
            for feature in self.selected_future_features
        ]
        selected_scalar_responses_indices = [
            self.all_scalar_responses.index(response)
            for response in self.selected_scalar_responses
        ]
        selected_relative_responses_indices = [
            self.all_relative_responses.index(response)
            for response in self.selected_relative_responses
        ]
        ...
        train_past_features = past_features[
            np.ix_(
                train_mask, np.arange(self.n_steps_in), selected_past_features_indices
            )
        ]
        validate_past_features = past_features[
            np.ix_(
                validate_mask,
                np.arange(self.n_steps_in),
                selected_past_features_indices,
            )
        ]
        train_future_features = future_features[
            np.ix_(
                train_mask,
                np.arange(self.n_steps_out),
                selected_future_features_indices,
            )
        ]
        validate_future_features = future_features[
            np.ix_(
                validate_mask,
                np.arange(self.n_steps_out),
                selected_future_features_indices,
        ...
        train_scalar_responses = scalar_responses[
            np.ix_(
                train_mask,
                np.arange(self.n_steps_out),
                selected_scalar_responses_indices,
        ...
        validate_scalar_responses = scalar_responses[
            np.ix_(
                validate_mask,
                np.arange(self.n_steps_out),
                selected_scalar_responses_indices,
        ...
        train_relative_responses = relative_responses[
            np.ix_(
                train_mask,
                np.arange(self.n_steps_out),
                selected_relative_responses_indices,
        ...
        validate_relative_responses = relative_responses[
            np.ix_(
                validate_mask,
                np.arange(self.n_steps_out),
                selected_relative_responses_indices,
        ...
        if verbose >= 2:
            ...
            print(
                f"train_clear_sky_irradiances.shape: {train_clear_sky_irradiances.shape}"
            )
            ...
            print(
                f"validate_relative_responses.shape: {validate_relative_responses.shape}"
            )
            print(
                f"validate_clear_sky_indexes.shape: {validate_clear_sky_indexes.shape}"
            )
            print(
                f"validate_clear_sky_irradiances.shape: {validate_clear_sky_irradiances.shape}"
            )
        ...
        for i, selected_feature in enumerate(self.selected_past_features):
            ...
            train_past_features[:, :, i] = feature_scaler.fit_transform(
                feature_data.reshape(-1, 1)
            ).reshape(feature_data_shape)
            validate_past_features_data = validate_past_features[:, :, i]
            validate_past_features_shape = validate_past_features_data.shape
            validate_past_features[:, :, i] = feature_scaler.transform(
                validate_past_features_data.reshape(-1, 1)
            ).reshape(validate_past_features_shape)
        ...
        for i, selected_feature in enumerate(self.selected_future_features):
            ...
            feature_data_shape = feature_data.shape
            train_future_features[:, :, i] = feature_scaler.fit_transform(
                feature_data.reshape(-1, 1)
            ).reshape(feature_data_shape)
            ...
            validate_future_features[:, :, i] = feature_scaler.transform(
                validate_future_features_data.reshape(-1, 1)
            ).reshape(validate_future_features_shape)
            self.scalers[selected_feature] = feature_scaler
        if self.scale_responses:
            for i, selected_response in enumerate(self.selected_scalar_responses):
                ...
                train_scalar_responses[:, :, i] = response_scaler.fit_transform(
                    response_data.reshape(-1, 1)
                ).reshape(response_data_shape)
                ...
                validate_scalar_responses[:, :, i] = response_scaler.transform(
                    validate_scalar_responses_data.reshape(-1, 1)
                ).reshape(validate_scalar_responses_shape)
                self.scalers[selected_response] = response_scaler
                self.response_scaler = (
                    response_scaler  # TODO make it so multiple responses can be scaled
                )
            for i, selected_response in enumerate(self.selected_relative_responses):
                response_scaler = self.scaler()
                response_data = train_relative_responses[:, :, i]
                response_data_shape = response_data.shape
                train_relative_responses[:, :, i] = response_scaler.fit_transform(
                    response_data.reshape(-1, 1)
                ).reshape(response_data_shape)
                validate_relative_responses_data = validate_relative_responses[:, :, i]
                validate_relative_responses_shape = (
                    validate_relative_responses_data.shape
                )
                validate_relative_responses[:, :, i] = response_scaler.transform(
                    validate_relative_responses_data.reshape(-1, 1)
                ).reshape(validate_relative_responses_shape)
                self.scalers[selected_response] = response_scaler
                self.response_scaler = (
                    response_scaler  # TODO make it so multiple responses can be scaled
                )
        ...
        if self.n_steps_in == 1:
            # self.train_past_features = np.expand_dims(train_past_features, axis=1)
        ...
        self.train_dates = train_datetimes
        if self.n_steps_in == 1:
            # self.validate_past_features = np.expand_dims(validate_past_features, axis=1)
    ...
    def create_default_model(self, n_steps_in, n_past_features, model_name):
        ...
        """
        layer_initializer = tf.keras.initializers.TruncatedNormal(
            mean=0.0, stddev=0.2, seed=self.seed
        )  # truncated normal initializer

        past_inputs = Input(shape=(n_steps_in, n_past_features), name="PastInputs")
        past_inputs = Dropout(
            self.dropout_ratio, input_shape=(n_steps_in, n_past_features)
        )(past_inputs)
        past_inputs_expanded = Lambda(
            lambda x: tf.expand_dims(x, axis=-1),
            input_shape=(n_steps_in, n_past_features),
            name="PastInputsExpanded",
        )(past_inputs)

        past_convolution = TimeDistributed(
            Conv1D(32, self.n_steps_in, padding="same", activation="relu")
        )(
            past_inputs_expanded
        )  # TODO check Kernel Size TODO SeperableConv1D TODO deep convolutions TODO Feed Conv results with unchanged inputs to deep LSTM
        ...
        x = Dense(
            self.n_steps_out * self.n_responses,
            activation="relu",
            kernel_initializer=layer_initializer,
        )(x)
        x = Reshape(
            (self.n_steps_out, self.n_responses),
            input_shape=(self.n_steps_out * self.n_responses,),
        )(x)
    ...
    def create_model_noise(self, n_steps_in, n_past_features, model_name):
        """
        Create tensorflow CNN-LSTM model.
        Inputs should be shape (n_samples, n_steps_in, n_past_features)
        Outputs should be shape (n_samples, n_steps_out, n_future_features)
        ...
        past_inputs = Input(shape=(n_steps_in, n_past_features), name="PastInputs")
        noise_inputs = Input(shape=(n_steps_in,), name="NoiseInputs")

        noise_inputs_expanded = Lambda(
            lambda x: tf.expand_dims(x, axis=-1),
            input_shape=(n_steps_in,),
            name="NoiseInputsExpanded",
        )(noise_inputs)
        noise_inputs_expanded = Dense(
            n_steps_in, kernel_initializer=layer_initializer, use_bias=False
        )(noise_inputs_expanded)
        past_inputs_concatenated = Concatenate(axis=-1)(
            [past_inputs, noise_inputs_expanded]
        )
        past_inputs_dropped = Dropout(
            self.dropout_ratio, input_shape=(n_steps_in, n_past_features + 1)
        )(past_inputs_concatenated)
        past_inputs_expanded = Lambda(
            lambda x: tf.expand_dims(x, axis=-1),
            input_shape=(n_steps_in, n_past_features + 1),
            name="PastInputsExpanded",
        )(past_inputs_dropped)
        ...
        self.model = tf.keras.Model(
            inputs=[past_inputs, noise_inputs], outputs=x, name=model_name
        )
    ...
    def compile_model(self):
        ...
        if self.model is None:
            if self.noise_model:
                self.model = self.create_model_noise(
                    n_steps_in=self.n_steps_in,
                    n_past_features=len(
                        self.selected_features
                    ),  # TODO account for future features
    ...
    def fit_model(self):
        ...
        history = self.model.fit(
            x=self.train_inputs,  # TODO save Model Weights in Neptune
            y=self.y_train_true,
            validation_data=(self.validation_inputs, self.y_validate_true),
            epochs=self.epochs,
        ...
        ### finish training on MAE loss ###
        weights = self.model.get_weights()
        self.loss = "mae"
        self.epochs = remaining_epochs
        self.compile_model()
    ...
    def rescaled_MAE_stepwise(self, y_true_scaled, y_predicted_scaled):
        ...
        y_predicted_rescaled = self.response_scaler.inverse_transform(
            y_predicted_scaled
        )
    ...
    def rescale_to_GHI(self, y, set_clear_sky_irradiances, set_clear_sky_indexes):
        ...
        # Convert to GHI
        if (
            "GHI" in self.selected_responses
        ):  # GHI wil already be accounted for TODO refactor for multiple responses TODO check logic
            y_rescaled = inverse_transform_y
        elif "CSI GHI" in self.selected_responses:
            y_rescaled = (
                inverse_transform_y * set_clear_sky_irradiances[:, self.n_steps_in :]
            )
            ...
            GHI_t0 = (
                set_clear_sky_irradiances[:, self.n_steps_in]
                * set_clear_sky_indexes[:, self.n_steps_in]
            )
            y_rescaled = inverse_transform_y + GHI_t0[:, None]
        elif "Delta CSI GHI" in self.selected_responses:
            y_rescaled = (
                inverse_transform_y + set_clear_sky_indexes[:, self.n_steps_in][:, None]
            ) * set_clear_sky_irradiances[:, self.n_steps_in :]
        else:
            ValueError(
                """selected_responses must include one of {"GHI", 'CSI GHI', 'cs_dev t ghi', "Delta GHI", "Delta CSI"}"""
            )
        return y_rescaled
    ...
    def final_error_metrics(self, cv):
        ...
        # rescale
        if self.scale_responses:
            self.y_train_predicted_rescaled = self.rescale_to_GHI(
                self.y_train_predicted,
                self.train_clear_sky_irradiances,
                self.train_clear_sky_indexes,
            )
            self.y_validate_predicted_rescaled = self.rescale_to_GHI(
                self.y_validate_predicted,
                self.validate_clear_sky_irradiances,
                self.validate_clear_sky_indexes,
            )

            self.y_train_true_rescaled = self.rescale_to_GHI(
                self.y_train_true,
                self.train_clear_sky_irradiances,
                self.train_clear_sky_indexes,
            )
            self.y_validate_true_rescaled = self.rescale_to_GHI(
                self.y_validate_true,
                self.validate_clear_sky_irradiances,
                self.validate_clear_sky_indexes,
            )

        else:  # or don't rescale
            self.y_train_predicted_rescaled = (
                self.y_train_predicted
            )  # [:,:,None]  # add third axis for predicted if not rescaling
        ...
        self.y_validate_predicted_rescaled = np.squeeze(
            self.y_validate_predicted_rescaled
        )
        ...
        # MBE
        self.run[f"{cv_string}Train MBE"] = utils.MBE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        )
        self.run[f"{cv_string}Validate MBE"] = utils.MBE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        )

        for i in range(12):
            self.run[f"{cv_string}Train MBE t+{i+1}0 min"] = utils.MBE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            )
            self.run[f"{cv_string}Validate MBE t+{i+1}0 min"] = utils.MBE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            )

        # nMBE
        self.run[f"{cv_string}Train nMBE"] = utils.nMBE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        )
        self.run[f"{cv_string}Validate nMBE"] = utils.nMBE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        )

        for i in range(12):
            self.run[f"{cv_string}Train nMBE t+{i+1}0 min"] = utils.nMBE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            )
            self.run[f"{cv_string}Validate nMBE t+{i+1}0 min"] = utils.nMBE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            )

        # MAE
        self.run[f"{cv_string}Train MAE"] = utils.MAE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        )
        self.run[f"{cv_string}Validate MAE"] = utils.MAE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        )

        for i in range(12):
            self.run[f"{cv_string}Train MAE t+{i+1}0 min"] = utils.MAE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            )
            self.run[f"{cv_string}Validate MAE t+{i+1}0 min"] = utils.MAE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            )

        # nMAE
        self.run[f"{cv_string}Train nMAE"] = utils.nMAE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        )
        self.run[f"{cv_string}Validate nMAE"] = utils.nMAE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        )

        for i in range(12):
            self.run[f"{cv_string}Train nMAE t+{i+1}0 min"] = utils.nMAE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            )
            self.run[f"{cv_string}Validate nMAE t+{i+1}0 min"] = utils.nMAE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            )

        # MSE
        self.run[f"{cv_string}Train MSE"] = utils.MSE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        )
        self.run[f"{cv_string}Validate MSE"] = utils.MSE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        )

        for i in range(12):
            self.run[f"{cv_string}Train MSE t+{i+1}0 min"] = utils.MSE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            )
            self.run[f"{cv_string}Validate MSE t+{i+1}0 min"] = utils.MSE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            )

        # RSME
        self.run[f"{cv_string}Train RMSE"] = utils.RMSE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        )
        self.run[f"{cv_string}Validate RMSE"] = utils.RMSE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        )

        for i in range(12):
            self.run[f"{cv_string}Train RMSE t+{i+1}0 min"] = utils.RMSE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            )
            self.run[f"{cv_string}Validate RMSE t+{i+1}0 min"] = utils.RMSE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            )

        # nRMSE
        self.run[f"{cv_string}Train nRMSE"] = utils.nRMSE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        )
        self.run[f"{cv_string}Validate nRMSE"] = utils.nRMSE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        )

        for i in range(12):
            self.run[f"{cv_string}Train nRMSE t+{i+1}0 min"] = utils.nRMSE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            )
            self.run[f"{cv_string}Validate nRMSE t+{i+1}0 min"] = utils.nRMSE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            )

        # Persistence
        self.train_poc_prediction = utils.persistence_of_cloudiness_prediction(
            self.train_clear_sky_indexes,
            self.train_clear_sky_irradiances,
            self.n_steps_in,
            self.n_steps_out,
        )
        self.validate_poc_prediction = utils.persistence_of_cloudiness_prediction(
            self.validate_clear_sky_indexes,
            self.validate_clear_sky_irradiances,
            self.n_steps_in,
            self.n_steps_out,
        ...
        ## Persistence MAE
        self.run[f"{cv_string}Train Persistence MAE"] = utils.MAE(
            self.y_train_true_rescaled, self.train_poc_prediction
        )
        self.run[f"{cv_string}Validate Persistence MAE"] = utils.MAE(
            self.y_validate_true_rescaled, self.validate_poc_prediction
        )

        for i in range(12):
            self.run[f"{cv_string}Train Persistence MAE t+{i+1}0 min"] = utils.MAE(
                self.y_train_true_rescaled[:, i], self.train_poc_prediction[:, i]
            )
            self.run[f"{cv_string}Validate Persistence MAE t+{i+1}0 min"] = utils.MAE(
                self.y_validate_true_rescaled[:, i], self.validate_poc_prediction[:, i]
            )

        ## Persistence nMAE
        self.run[f"{cv_string}Train Persistence nMAE"] = utils.nMAE(
            self.y_train_true_rescaled, self.train_poc_prediction
        )
        self.run[f"{cv_string}Validate Persistence nMAE"] = utils.nMAE(
            self.y_validate_true_rescaled, self.validate_poc_prediction
        )

        for i in range(12):
            self.run[f"{cv_string}Train Persistence nMAE t+{i+1}0 min"] = utils.nMAE(
                self.y_train_true_rescaled[:, i], self.train_poc_prediction[:, i]
            )
            self.run[f"{cv_string}Validate Persistence nMAE t+{i+1}0 min"] = utils.nMAE(
                self.y_validate_true_rescaled[:, i], self.validate_poc_prediction[:, i]
            )

        ## Persistence MSE
        self.run[f"{cv_string}Train Persistence MSE"] = utils.MSE(
            self.y_train_true_rescaled, self.train_poc_prediction
        )
        self.run[f"{cv_string}Validate Persistence MSE"] = utils.MSE(
            self.y_validate_true_rescaled, self.validate_poc_prediction
        )

        for i in range(12):
            self.run[f"{cv_string}Train Persistence MSE t+{i+1}0 min"] = utils.MSE(
                self.y_train_true_rescaled[:, i], self.train_poc_prediction[:, i]
            )
            self.run[f"{cv_string}Validate Persistence MSE t+{i+1}0 min"] = utils.MSE(
                self.y_validate_true_rescaled[:, i], self.validate_poc_prediction[:, i]
            )

        ## Persistence RSME
        self.run[f"{cv_string}Train Persistence RMSE"] = utils.RMSE(
            self.y_train_true_rescaled, self.train_poc_prediction
        )
        self.run[f"{cv_string}Validate Persistence RMSE"] = utils.RMSE(
            self.y_validate_true_rescaled, self.validate_poc_prediction
        )

        for i in range(12):
            self.run[f"{cv_string}Train Persistence RMSE t+{i+1}0 min"] = utils.RMSE(
                self.y_train_true_rescaled[:, i], self.train_poc_prediction[:, i]
            )
            self.run[f"{cv_string}Validate Persistence RMSE t+{i+1}0 min"] = utils.RMSE(
                self.y_validate_true_rescaled[:, i], self.validate_poc_prediction[:, i]
            )

        ## Persistence nRMSE
        self.run[f"{cv_string}Train Persistence nRMSE"] = utils.nRMSE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        )
        self.run[f"{cv_string}Validate Persistence nRMSE"] = utils.nRMSE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        )

        for i in range(12):
            self.run[f"{cv_string}Train Persistence nRMSE t+{i+1}0 min"] = utils.nRMSE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            )
            self.run[f"{cv_string}Validate Persistence nRMSE t+{i+1}0 min"] = (
                utils.nRMSE(
                    self.y_validate_true_rescaled[:, i],
                    self.y_validate_predicted_rescaled[:, i],
                )
        ...
        ## FS MAE
        self.run[f"{cv_string}Train FS MAE"] = 1 - utils.MAE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        ) / utils.MAE(self.y_train_true_rescaled, self.train_poc_prediction)
        self.run[f"{cv_string}Validate FS MAE"] = 1 - utils.MAE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        ) / utils.MAE(self.y_validate_true_rescaled, self.validate_poc_prediction)

        for i in range(12):
            self.run[f"{cv_string}Train FS MAE t+{i+1}0 min"] = 1 - utils.MAE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            ) / utils.MAE(
                self.y_train_true_rescaled[:, i], self.train_poc_prediction[:, i]
            )
            self.run[f"{cv_string}Validate FS MAE t+{i+1}0 min"] = 1 - utils.MAE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            ) / utils.MAE(
                self.y_validate_true_rescaled[:, i], self.validate_poc_prediction[:, i]
        ...
        ## FS nMAE
        self.run[f"{cv_string}Train FS nMAE"] = 1 - utils.nMAE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        ) / utils.MAE(self.y_train_true_rescaled, self.train_poc_prediction)
        self.run[f"{cv_string}Validate FS nMAE"] = 1 - utils.nMAE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        ) / utils.MAE(self.y_validate_true_rescaled, self.validate_poc_prediction)

        for i in range(12):
            self.run[f"{cv_string}Train FS nMAE t+{i+1}0 min"] = 1 - utils.nMAE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            ) / utils.MAE(
                self.y_train_true_rescaled[:, i], self.train_poc_prediction[:, i]
            )
            self.run[f"{cv_string}Validate FS nMAE t+{i+1}0 min"] = 1 - utils.nMAE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            ) / utils.MAE(
                self.y_validate_true_rescaled[:, i], self.validate_poc_prediction[:, i]
        ...
        ## FS MSE
        self.run[f"{cv_string}Train FS MSE"] = 1 - utils.MSE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        ) / utils.MAE(self.y_train_true_rescaled, self.train_poc_prediction)
        self.run[f"{cv_string}Validate FS MSE"] = 1 - utils.MSE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        ) / utils.MAE(self.y_validate_true_rescaled, self.validate_poc_prediction)

        for i in range(12):
            self.run[f"{cv_string}Train FS MSE t+{i+1}0 min"] = 1 - utils.MSE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            ) / utils.MAE(
                self.y_train_true_rescaled[:, i], self.train_poc_prediction[:, i]
            )
            self.run[f"{cv_string}Validate FS MSE t+{i+1}0 min"] = 1 - utils.MSE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            ) / utils.MAE(
                self.y_validate_true_rescaled[:, i], self.validate_poc_prediction[:, i]
        ...
        ## FS RSME
        self.run[f"{cv_string}Train FS RMSE"] = 1 - utils.RMSE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        ) / utils.MAE(self.y_train_true_rescaled, self.train_poc_prediction)
        self.run[f"{cv_string}Validate FS RMSE"] = 1 - utils.RMSE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        ) / utils.MAE(self.y_validate_true_rescaled, self.validate_poc_prediction)

        for i in range(12):
            self.run[f"{cv_string}Train FS RMSE t+{i+1}0 min"] = 1 - utils.RMSE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            ) / utils.MAE(
                self.y_train_true_rescaled[:, i], self.train_poc_prediction[:, i]
            )
            self.run[f"{cv_string}Validate FS RMSE t+{i+1}0 min"] = 1 - utils.RMSE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            ) / utils.MAE(
                self.y_validate_true_rescaled[:, i], self.validate_poc_prediction[:, i]
        ...
        ## FS nRMSE
        self.run[f"{cv_string}Train FS nRMSE"] = 1 - utils.nRMSE(
            self.y_train_true_rescaled, self.y_train_predicted_rescaled
        ) / utils.MAE(self.y_train_true_rescaled, self.train_poc_prediction)
        self.run[f"{cv_string}Validate FS nRMSE"] = 1 - utils.nRMSE(
            self.y_validate_true_rescaled, self.y_validate_predicted_rescaled
        ) / utils.MAE(self.y_validate_true_rescaled, self.validate_poc_prediction)

        for i in range(12):
            self.run[f"{cv_string}Train FS nRMSE t+{i+1}0 min"] = 1 - utils.nRMSE(
                self.y_train_true_rescaled[:, i], self.y_train_predicted_rescaled[:, i]
            ) / utils.MAE(
                self.y_train_true_rescaled[:, i], self.train_poc_prediction[:, i]
            )
            self.run[f"{cv_string}Validate FS nRMSE t+{i+1}0 min"] = 1 - utils.nRMSE(
                self.y_validate_true_rescaled[:, i],
                self.y_validate_predicted_rescaled[:, i],
            ) / utils.MAE(
                self.y_validate_true_rescaled[:, i], self.validate_poc_prediction[:, i]
    ...
    def save_and_quit(self, cv):
        if self.model_save_path is not None:
            ...
            error_path = os.path.join(
                self.model_save_path, my_run_id + cv_string + "_residuals.h5"
            )
            ...
            train_group.create_dataset(
                "predicted", data=self.y_train_predicted_rescaled
            )
            ...
            validate_group.create_dataset(
                "predicted", data=self.y_validate_predicted_rescaled
            )
            validate_group.create_dataset(
                "persistence", data=self.validate_poc_prediction
            )
    ...
    def do_it_all(
        ...
        if not cached:
            ...
            self.preprocess_joint_data(
                train_validate_date, end_date=end_date, verbose=verbose
            )
            if verbose:
            ...
            if verbose:
                print(" Using Cached Data ".center(40, "-"))
            self.import_preprocess_cached_windows(
                train_validate_date, end_date, verbose
            )
            if verbose:
                print(" Preprocessing Complete ".center(40, "-"))
        if verbose:
    ...
    def cross_validated_study(
        ...
        if isinstance(self.learning_rate, tf.optimizers.schedules.LearningRateSchedule):
            saved_lr = tf.keras.optimizers.schedules.serialize(self.learning_rate)
        else:
            saved_lr = self.learning_rate
        for i in range(len(train_validate_dates)):
            if verbose:
                print(f" Starting CV {i+1} ".center(40, "="))
            self.learning_rate = tf.keras.optimizers.schedules.deserialize(saved_lr)
            self.do_it_all(
                train_validate_dates[i],
                end_dates[i],
                cv=i + 1,
                verbose=verbose,
                cached=cached,
